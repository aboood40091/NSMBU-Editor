...
struct MTX34
{
    ...
    static MTX34* Concat(MTX34* dst, const MTX34* a, const MTX34* b)
    {
        ...
        dst->m[0][0] = a11 * b11 + a12 * b21 + a13 * b31;
        dst->m[0][1] = a11 * b12 + a12 * b22 + a13 * b32;
        dst->m[0][2] = a11 * b13 + a12 * b23 + a13 * b33;
        dst->m[0][3] = a11 * b14 + a12 * b24 + a13 * b34 + a->m[0][3];

        dst->m[1][0] = a21 * b11 + a22 * b21 + a23 * b31;
        dst->m[1][1] = a21 * b12 + a22 * b22 + a23 * b32;
        dst->m[1][2] = a21 * b13 + a22 * b23 + a23 * b33;
        dst->m[1][3] = a21 * b14 + a22 * b24 + a23 * b34 + a->m[1][3];

        dst->m[2][0] = a31 * b11 + a32 * b21 + a33 * b31;
        dst->m[2][1] = a31 * b12 + a32 * b22 + a33 * b32;
        dst->m[2][2] = a31 * b13 + a32 * b23 + a33 * b33;
        dst->m[2][3] = a31 * b14 + a32 * b24 + a33 * b34 + a->m[2][3];
    ...
    static MTX34* Inverse(MTX34* dst, const MTX34* src)
    {
        ...
        const f32 a11 = src->m[0][0];
        const f32 a12 = src->m[0][1];
        const f32 a13 = src->m[0][2];
        const f32 a14 = src->m[0][3];

        const f32 a21 = src->m[1][0];
        const f32 a22 = src->m[1][1];
        const f32 a23 = src->m[1][2];
        const f32 a24 = src->m[1][3];

        const f32 a31 = src->m[2][0];
        const f32 a32 = src->m[2][1];
        const f32 a33 = src->m[2][2];
        const f32 a34 = src->m[2][3];

        f32 det = (a11 * a22 * a33 - a31 * a22 * a13)
                + (a12 * a23 * a31 - a21 * a12 * a33)
                + (a13 * a21 * a32 - a11 * a32 * a23);

        if (det == 0)
            return Copy(dst, &Identity());

        det = 1 / det;

        dst->m[0][0] = (a22 * a33 - a32 * a23) * det;
        dst->m[0][1] = (a32 * a13 - a12 * a33) * det;
        dst->m[0][2] = (a12 * a23 - a22 * a13) * det;

        dst->m[1][0] = (a31 * a23 - a21 * a33) * det;
        dst->m[1][1] = (a11 * a33 - a31 * a13) * det;
        dst->m[1][2] = (a21 * a13 - a11 * a23) * det;

        dst->m[2][0] = (a21 * a32 - a31 * a22) * det;
        dst->m[2][1] = (a31 * a12 - a11 * a32) * det;
        dst->m[2][2] = (a11 * a22 - a21 * a12) * det;

        dst->m[0][3] = dst->m[0][0] * -a14 + dst->m[0][1] * -a24 + dst->m[0][2] * -a34;
        dst->m[1][3] = dst->m[1][0] * -a14 + dst->m[1][1] * -a24 + dst->m[1][2] * -a34;
        dst->m[2][3] = dst->m[2][0] * -a14 + dst->m[2][1] * -a24 + dst->m[2][2] * -a34;
#else
