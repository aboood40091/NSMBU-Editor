...
static forceinline bool ReadFile(const char* filename, u8** out_data, u32* out_size)
{
...
static forceinline bool WriteFile(const char* filename, u8* data, u32 size)
{
    if (!data || !size)
        return false;
...
static forceinline void FreeFile(const void* data)
{
    delete[] (u8*)data;
}

static forceinline bool ReadFile(const std::string& filename, std::string* out_str)
{
    if (!out_str)
        return false;
...
static inline bool WriteFile(const std::string& filename, const std::string& str)
{
