...
u32 Renderer::MakeConnectionStripeAttributeBlockCore(EmitterInstance* emitter, s32 numParticles, PtclInstance* ptclLast, PtclInstance* ptclBeforeLast, s32 connectionType, StripeVertexBuffer* stripeVertexBuffer, s32 firstVertex)
{
    ...
    currentSliceDir.x = 0.0f;
    currentSliceDir.y = 0.0f;
    currentSliceDir.z = 0.0f;
...
bool Renderer::SetupStripeDrawSetting(const EmitterInstance* emitter, bool flushCache, void* argData)
{
    ...
    if (shader->fragmentShaderKey.softEdge && this->textures[0] != EFT_NATIVE_TEXTURE_NONE)
        renderContext.SetupTexture(this->textures[0], TextureSlot_2, shader->fragmentSamplerLocations2[0]);

    if (shader->fragmentShaderKey.shaderMode == 1 && this->textures[1] != EFT_NATIVE_TEXTURE_NONE)
...
void Renderer::EntryConnectionStripe(EmitterInstance* emitter, bool flushCache, void* argData)
{
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(StripeVertexBuffer) * emitter->numDrawStripe, stripeVertexBuffer, GL_STATIC_DRAW);

    VertexBuffer::BindExtBuffer(shader->attrPosBuffer,      4, sizeof(StripeVertexBuffer),  0 * 4);
    VertexBuffer::BindExtBuffer(shader->attrOuterBuffer,    4, sizeof(StripeVertexBuffer),  4 * 4);
    VertexBuffer::BindExtBuffer(shader->attrTexCoordBuffer, 4, sizeof(StripeVertexBuffer),  8 * 4);
    VertexBuffer::BindExtBuffer(shader->attrDirBuffer,      4, sizeof(StripeVertexBuffer), 12 * 4);
#else
...

    {
        ...
        Drawer::DrawArrays(Drawer::TRIANGLE_STRIP, numDrawStripe);
    }
...
void Renderer::EntryStripe(EmitterInstance* emitter, bool flushCache, void* argData)
{
    ...
    for (PtclInstance* ptcl = emitter->particleHead; ptcl != NULL; ptcl = ptcl->next)
    {
...

        {
            ...
            shader->stripeUniformBlock.BindUniformBlock(uniformBlock);
            Drawer::DrawArrays(Drawer::TRIANGLE_STRIP, ptcl->stripe->numDraw, ptcl->stripe->drawFirstVertex);
        }
