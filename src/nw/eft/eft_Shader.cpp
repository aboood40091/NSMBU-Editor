...
void ParticleShader::EnableInstanced()
{
    ...
    if (attrWldPosBuffer != 0xFFFFFFFF)
        glVertexAttribDivisor(attrWldPosBuffer, 1);

    if (attrSclBuffer != 0xFFFFFFFF)
        glVertexAttribDivisor(attrSclBuffer, 1);

    if (attrColor0Buffer != 0xFFFFFFFF)
        glVertexAttribDivisor(attrColor0Buffer, 1);

    if (attrColor1Buffer != 0xFFFFFFFF)
        glVertexAttribDivisor(attrColor1Buffer, 1);

    if (attrTexAnimBuffer != 0xFFFFFFFF)
        glVertexAttribDivisor(attrTexAnimBuffer, 1);

    if (attrWldPosDfBuffer != 0xFFFFFFFF)
        glVertexAttribDivisor(attrWldPosDfBuffer, 1);

    if (attrRotBuffer != 0xFFFFFFFF)
        glVertexAttribDivisor(attrRotBuffer, 1);

    if (attrSubTexAnimBuffer != 0xFFFFFFFF)
        glVertexAttribDivisor(attrSubTexAnimBuffer, 1);

    if (attrEmMat0Buffer != 0xFFFFFFFF)
        glVertexAttribDivisor(attrEmMat0Buffer, 1);

    if (attrEmMat1Buffer != 0xFFFFFFFF)
        glVertexAttribDivisor(attrEmMat1Buffer, 1);

    if (attrEmMat2Buffer != 0xFFFFFFFF)
        glVertexAttribDivisor(attrEmMat2Buffer, 1);
#endif // EFT_WIN
}
...
#ifdef EFT_WIN
    if (attrWldPosBuffer != 0xFFFFFFFF)
        glVertexAttribDivisor(attrWldPosBuffer, 0);

    if (attrSclBuffer != 0xFFFFFFFF)
        glVertexAttribDivisor(attrSclBuffer, 0);

    if (attrColor0Buffer != 0xFFFFFFFF)
        glVertexAttribDivisor(attrColor0Buffer, 0);

    if (attrColor1Buffer != 0xFFFFFFFF)
        glVertexAttribDivisor(attrColor1Buffer, 0);

    if (attrTexAnimBuffer != 0xFFFFFFFF)
        glVertexAttribDivisor(attrTexAnimBuffer, 0);

    if (attrWldPosDfBuffer != 0xFFFFFFFF)
        glVertexAttribDivisor(attrWldPosDfBuffer, 0);

    if (attrRotBuffer != 0xFFFFFFFF)
        glVertexAttribDivisor(attrRotBuffer, 0);

    if (attrSubTexAnimBuffer != 0xFFFFFFFF)
        glVertexAttribDivisor(attrSubTexAnimBuffer, 0);

    if (attrEmMat0Buffer != 0xFFFFFFFF)
        glVertexAttribDivisor(attrEmMat0Buffer, 0);

    if (attrEmMat1Buffer != 0xFFFFFFFF)
        glVertexAttribDivisor(attrEmMat1Buffer, 0);

    if (attrEmMat2Buffer != 0xFFFFFFFF)
        glVertexAttribDivisor(attrEmMat2Buffer, 0);
#endif // EFT_WIN
}
